...
globalThis.fetch = async (url, opts) => {
  ...
  if (u.includes('/v1/images/generations') && u.includes('selfhost.test')) {
    ...
    assert.strictEqual(b.async, true, 'selfhost wajib mode async (anti 504)');
    return jsonResp({ task_id: 'gt-123', status: 'processing' });
  }

  // --- Self-Host: polling status task async ---
  if (u.includes('/v1/tasks/') && u.includes('selfhost.test')) {
    assert.strictEqual(method, 'GET', 'status task pakai GET');
    const PNG = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==';
    return jsonResp({ task_id: 'gt-123', status: 'completed', data: [{ b64_json: PNG }, { b64_json: PNG }] });
  }
...
console.log('Self-Host (gateway ComfyUI + LoRA):');
{
  ...
  assert.strictEqual(g.data.images.length, 0, 'async: generate balas cepat tanpa gambar (taskId saja)');
  ok('generate selfhost (async) -> taskId selfhost:... balas < 2s');

  const t = await run('/api/task?id=' + encodeURIComponent(g.data.taskId), null, { IMAGES });
  assert.strictEqual(t.data.status, 'SUCCESS');
  assert.ok(Array.isArray(t.data.images) && t.data.images.length === 2, '2 gambar diarsip dari gateway');
  assert.ok(t.data.images[0].startsWith('/img/'), 'gambar selfhost diarsip ke /img/');
  ok('task selfhost -> SUCCESS + 2 gambar /img/ (poll gateway)');
...
console.log('Validasi (API key + provider salah):');
{
  ...
  assert.deepStrictEqual(h.data.hasKeys, { tams: false, replicate: true, fal: false, pollinations: true, selfhost: true });
