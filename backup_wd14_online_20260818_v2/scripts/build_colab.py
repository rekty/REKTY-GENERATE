...
KEEP_ALIVE = code(
    ...
    "# karena nganggur. Tiap 45 detik: cek ComfyUI (8188) + gateway (8000) + tunnel;",
    "# kalau ada yang mati, restart otomatis.",
    "import time, subprocess, sys, os, re, urllib.request",
    "",
    'COMFY = WORK + "/ComfyUI"',
    'GW = WORK + "/gateway.py"',
    "",
    ...
    "",
    "def _tunnel_alive():",
    '    # Proses cloudflared masih hidup?',
    "    try:",
    ...
    "        return False",
    "",
    "def _tunnel_public_ok():",
    '    # URL publik benar-benar menjawab /health?',
    "    ep = _endpoint()",
    "    if not ep:",
    "        return False",
    "    try:",
    '        r = urllib.request.urlopen(ep + "/health", timeout=8)',
    "        return r.status == 200",
    "    except Exception:",
    "        return False",
    "",
    "def _start_tunnel():",
    '    # Matikan cloudflared lama, mulai baru, tunggu URL baru muncul.',
    "    try:",
    ...
    '                with open(WORK + "/endpoint.txt", "w") as f:',
    "                    f.write(m.group(0))",
    '                print(time.strftime("%H:%M:%S"), "TUNNEL RESTART - URL BARU:", m.group(0), flush=True)',
    "                return True",
    "        except Exception:",
    "            pass",
    '    print(time.strftime("%H:%M:%S"), "TUNNEL RESTART - URL belum muncul, cek tunnel.log", flush=True)',
    "    return False",
    "",
    "t0 = time.time()",
    ...
    "        _start_gateway()",
    "    # Tunnel: pastikan proses hidup DAN endpoint publik menjawab",
    "    if not _tunnel_alive():",
    '        print(time.strftime("%H:%M:%S"), "tunnel mati -> restart otomatis", flush=True)',
    "        _start_tunnel()",
    "    elif _up(8000) and not _tunnel_public_ok():",
    '        print(time.strftime("%H:%M:%S"), "tunnel hidup tapi URL publik mati -> restart otomatis", flush=True)',
    "        _start_tunnel()",
